...
async function load() {
  try {
    ...
    const migrated = await AuthService.prepareAccounts(state.accounts);
    if (migrated) {
      save();
    }
  } catch (e) {
  ...
  if (!state.accounts.find(acc => acc.username === 'admin')) {
    const admin = await AuthService.createAccount({
      username: 'admin',
      password: '1234',
      name: 'Admin User',
      email: 'admin@energyiq.app',
      address: '',
      plan: 'Free',
    });
...
function getLastLoginLabel() {
  const ts = StorageService.load(StorageService.keys.lastLogin, '');
  if (!ts) return '—';
...
// Inject hidden dummy fields to absorb late browser autofill (Safari workaround)
...
function prefillRememberedLogin() {
  const remembered = StorageService.load(StorageService.keys.rememberUsername, '') || '';
...
async function handleLogin() {
  ...
  const account = await AuthService.authenticate(state.accounts, uname, pw);
  ...
  if (account) {
    ...
    if (remember) {
      ...
      StorageService.remove(StorageService.keys.rememberUsername);
    }
    StorageService.save(StorageService.keys.lastLogin, new Date().toISOString());
    save();
    showApp();
  } else {
    if (errEl) {
...
function renderAutomation() {
  ...
  if (sel) {
    if (!hasDevices) {
      sel.innerHTML = '<option value="">No devices found. Add a device first.</option>';
      sel.disabled = true;
    } else {
      sel.disabled = false;
  ...
  if (saveBtn) {
    saveBtn.disabled = !hasDevices;
    saveBtn.classList.toggle('btn-disabled', !hasDevices);
  }

  if (error) {
    if (!hasDevices) {
      error.textContent = 'Add a device before creating an automation rule.';
  ...
  if (rulesList) {
    if (state.automationRules.length === 0) {
      rulesList.innerHTML = '<p style="color:var(--text-muted);">No automation rules yet. Create one after adding a device.</p>';
    } else {
      rulesList.innerHTML = state.automationRules.map((r, idx) => `
        <div style="padding:10px;background:var(--bg-elevated);border-radius:var(--radius-sm);margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;gap:12px;">
          <div style="font-size:0.85rem;">
            <strong>${escHtml(getDeviceName(r.device) || 'Unknown device')}</strong>
            <span style="display:block;color:var(--text-secondary);margin-top:4px;">${formatAutomationAction(r.action)} at ${r.time} on ${r.days.join(', ')}</span>
          </div>
...
function getDeviceName(deviceId) {
  return state.devices.find(d => d.id === deviceId)?.name;
}

function formatAutomationAction(action) {
  if (action === 'turnoff') return 'Turn off';
  if (action === 'reduce') return 'Reduce to 50%';
  return action;
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Also attempt to intercept autofill by creating hidden dummy fields
